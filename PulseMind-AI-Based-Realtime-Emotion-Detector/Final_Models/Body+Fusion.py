...
# =========================
# MediaPipe is currently having import issues, so we'll disable pose detection for now
MEDIAPIPE_AVAILABLE = False
mp_drawing = None
mp_pose = None

print("⚠ MediaPipe pose detection disabled due to import issues")
print("✓ Script will continue with emotion detection only")
...
class UnifiedBehaviorEmotionDetector:
    ...
    def detect_pose_and_gesture(self, image, results):
        if not MEDIAPIPE_AVAILABLE or results is None:
            self.current_results['posture'] = 'Unknown'
            self.current_results['gesture_summary'] = 'Pose detection unavailable'
            return
    ...
    def run(self):
        ...
        print("Note: Running without pose detection due to MediaPipe issues")
        ...
        while self.cap.isOpened():
            ...
            # Skip pose detection and use default values
            self.current_results['posture'] = 'Unknown (pose detection disabled)'
            self.current_results['gesture_summary'] = 'Pose detection unavailable'
            ...
            cv2.imshow('Unified Behavioral Feedback (No Pose)', frame)
            print(fused_text)
